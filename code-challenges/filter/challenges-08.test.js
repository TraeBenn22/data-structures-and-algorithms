...
const filterStringsWithVowels = (arr) => {
    ...
    const newArray = arr.filter(vowels => {
        for (let i = 0; i < vowelArray.length; i++) {
            if (vowels.includes(vowelArray[i]))
                return vowels
        }
...
const getStatName = (arr, minBaseStat) => {
    const newArray = getBaseStatGreaterThan(arr, minBaseStat);
    const myArray = newArray.map(element => {
